...
glm::mat4 getLocalToWorldMatrix(const tinygltf::Node &node, const glm::mat4 &parentMatrix)
{
  ...
  {
    return parentMatrix * glm::mat4(node.matrix[0], node.matrix[1], node.matrix[2], node.matrix[3], node.matrix[4], node.matrix[5],
  ...
  const auto rotationQuat = node.rotation.empty() ? glm::quat(1, 0, 0, 0)
      ...
      if (node.mesh >= 0)
      {
        ...
        for (size_t pIdx = 0; pIdx < mesh.primitives.size(); ++pIdx)
        {
          ...
          if (primitive.indices >= 0)
          {
            ...
            {
            default:
              std::cerr << "Primitive index accessor with bad componentType " << indexAccessor.componentType << ", skipping it."
                        << std::endl;
              continue;
